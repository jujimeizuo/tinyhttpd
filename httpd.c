...
void accept_request(void *arg) {
	...
	// 默认地址，解析道的路径如果是/，则自动加上index.html
	...
	if (stat(path, &st) == -1) {
		...
		// 如果你的文件默认是有执行全县的，自动解析成cgi程序，如果有执行权限但是不能执行，会接受到报错信号
...
void bad_request(int client) {
	...
	sprintf(buf, "HTTP/1.0 500 BAD REQUEST\r\n");
...
void execute_cgi(int client, const char *path,
	...
	if (strcasecmp(method, "GET") == 0) {
		...
		numchars = get_line(client, buf, sizeof(buf));
		while ((numchars > 0) && strcmp("\n", buf)) {
			// 如果是POST请求，就需要得到Content-Length，Content-Length：这个字符串一共长15位，所以
			// 取出头部一句后，将第16位设置结束符，进行比较
			// 第16位置为结束
			buf[15] = '\0';
			if (strcasecmp(buf, "Content-Length:") == 0) {
				// 内存从第17位开始就是长度，将17位开始的所有字符串转成整数就是content-length
	...
	if (pid == 0) { /*child: CGI script */
		...
		putenv(meth_env);
		if (strcasecmp(method, "GET") == 0) {
			sprintf(query_env, "REQUEST_STRING=%s", query_string);
			putenv(query_env);
		} else { /* POST */
